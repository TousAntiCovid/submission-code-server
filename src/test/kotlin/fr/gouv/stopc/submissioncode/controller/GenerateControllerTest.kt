...
class GenerateControllerTest(@Autowired val submissionCodeRepository: SubmissionCodeRepository) {
    ...
    fun can_generate_a_short_code() {
        ...
        assertThat(generatedCode.dateGeneration.toInstant(), within(5, SECONDS, now()))
        assertThat(generatedCode.dateAvailable.toInstant(), within(5, SECONDS, now()))
        assertThat(generatedCode.dateEndValidity.toInstant(), within(5, SECONDS, now().plus(1, HOURS)))
    ...
    fun can_generate_a_test_code() {
        ...
        assertThat(generatedCode.dateGeneration.toInstant(), within(5, SECONDS, now()))
        assertThat(generatedCode.dateAvailable.toInstant(), within(5, SECONDS, now()))
        assertThat(generatedCode.dateEndValidity.toInstant(), within(5, SECONDS, now().plus(15, DAYS).truncatedTo(DAYS)))
        assertThat(generatedCode.dateUse, nullValue())
        assertThat(generatedCode.used, equalTo(false))
    }
}
